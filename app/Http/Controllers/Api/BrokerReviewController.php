...
{
    public function index(Request $request, int $brokerId): JsonResponse
    {
        $broker = Broker::findOrFail($brokerId);

        $query = $broker->reviews()
    ...
    public function show(int $brokerId, int $reviewId): JsonResponse
    {
        $review = BrokerReview::where('broker_id', $brokerId)
            ->where('id', $reviewId)
            ->where('is_approved', true)
    ...
    public function store(Request $request, int $brokerId): JsonResponse
    {
        $broker = Broker::findOrFail($brokerId);
        $user = $request->user();
        ...
        $review = BrokerReview::create($data);
    ...
    public function update(Request $request, int $brokerId, int $reviewId): JsonResponse
    {
        $review = BrokerReview::where('broker_id', $brokerId)
            ->where('id', $reviewId)
    ...
    public function destroy(int $brokerId, int $reviewId, Request $request): JsonResponse
    {
        $review = BrokerReview::where('broker_id', $brokerId)
            ->where('id', $reviewId)
            ->where('user_id', $request->user()->id)
    ...
    public function vote(Request $request, int $brokerId, int $reviewId): JsonResponse
    {
        $review = BrokerReview::where('broker_id', $brokerId)
            ->where('id', $reviewId)
            ->where('is_approved', true)
