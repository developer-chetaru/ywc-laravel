...
class CareerHistoryApiController extends Controller
{
    ...
    public function uploadDocumentdfgdgdfg(Request $request)
    {
        try {
            $documentType = strtolower($request->input('document_type'));

            if (!$documentType) {
                throw ValidationException::withMessages(['document_type' => 'Document type is required']);
            }
            ...
            switch ($documentType) {
                ...
                case 'certificate':
                    $rules = array_merge($rules, [
                        ...
                        'certificates.*.certificate_type'     => 'required|string',
                        'certificates.*.certificate_issuer'   => 'required|string',
                    ]);
                    break;
            ...
            $documentData = [
                'document_type' => ucfirst($documentType),
                'file_path'     => $filePath,
            ];

            // Merge any other dynamic inputs
