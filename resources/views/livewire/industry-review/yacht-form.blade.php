        ...
        <div class="bg-white shadow-lg rounded-xl p-4 sm:p-6 border border-gray-200">
            ...
            <form wire:submit.prevent="save" class="space-y-4 sm:space-y-6">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mb-4">Organize images by labels/categories. Each label can contain multiple images.</p>
                    
                    @php
                        // Group existing gallery by label
                        $existing_by_label = [];
                        foreach($existing_gallery as $item) {
                            $label = $item['category'] ?? 'other';
                            if (!isset($existing_by_label[$label])) {
                    ...
                    {{-- Existing Gallery Images Grouped by Label --}}
                    @if(count($existing_by_label) > 0)
                        @foreach($existing_by_label as $label => $items)
                            <div class="mb-6 border border-gray-200 rounded-lg p-4 bg-gray-50">
                                <div class="flex items-center justify-between mb-3">
                                    <h3 class="text-sm font-semibold text-gray-700 uppercase">{{ $available_labels[$label] ?? ucfirst($label) }}</h3>
                                    <span class="text-xs text-gray-500">{{ count($items) }} image(s)</span>
                                </div>
                                    ...
                                    @foreach($items as $item)
                                        @if(!empty($item['url']))
                                            <div class="border border-gray-200 rounded-lg p-3 bg-white">
                                                ...
                                                <select wire:model="existing_gallery_labels.{{ $item['id'] }}"
                                                        class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 mb-2">
                                                    @foreach($available_labels as $key => $labelName)
                                                        <option value="{{ $key }}">{{ $labelName }}</option>
                                                    @endforeach
                                                </select>
                                                <input type="text" 
                    ...
                    @if(count($gallery_previews) > 0)
                        <div class="mb-6 border border-gray-200 rounded-lg p-4 bg-blue-50">
                            <h3 class="text-sm font-semibold text-gray-700 mb-3">New Images</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                @foreach($gallery_previews as $index => $preview)
                                    <div class="border border-gray-200 rounded-lg p-3 bg-white">
                                        ...
                                        <select wire:model="gallery_labels.{{ $index }}"
                                                class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 mb-2">
                                            @foreach($available_labels as $key => $labelName)
                                                <option value="{{ $key }}">{{ $labelName }}</option>
                                            @endforeach
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs text-gray-500 whitespace-nowrap">Max 20 images</span>
                    </div>
                    @error('gallery_images.*') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    <p class="text-xs text-gray-500 mt-1">You can upload multiple images at once. Select a label for each image and add a caption. Click the X button to remove any image.</p>
                </div>
