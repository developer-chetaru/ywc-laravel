@php
    ...
    $galleryImages = $restaurant->gallery->filter(function($img) {
...
<div class="py-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen" x-data="{ 
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        {{-- Back Button --}}
        <a href="{{ route('restaurant-reviews.show', $restaurant->slug) }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-blue-600 mb-6 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
            <span class="font-medium">Back to {{ $restaurant->name }}</span>
        </a>

        <div class="bg-white shadow-lg rounded-xl p-6 border border-gray-200">
            <h1 class="text-3xl font-bold text-gray-900 mb-6">Photo Gallery - {{ $restaurant->name }}</h1>
