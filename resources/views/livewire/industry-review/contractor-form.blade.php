    ...
    <div class="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8">
        <div class="bg-white shadow-lg rounded-xl p-4 sm:p-6 border border-gray-200">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">{{ $isEditMode ? 'Edit Contractor' : 'Add New Contractor' }}</h1>
                    <p class="text-sm text-gray-600">{{ $isEditMode ? 'Update contractor information' : 'Add a new contractor to the system' }}</p>
                </div>
                <a href="{{ route('industryreview.contractors.manage') }}" 
                   class="inline-flex items-center px-3 sm:px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                    ...
                    <span class="hidden sm:inline">Back to Contractors</span>
            ...
            <form wire:submit.prevent="save" class="space-y-4 sm:space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select wire:model="category" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="location" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" wire:model="specialtiesInput" placeholder="e.g., Radar installation, GPS upgrade" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Languages (comma-separated)</label>
                        <input type="text" wire:model="languagesInput" placeholder="e.g., English, French, Spanish" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Response Time</label>
                        <input type="text" wire:model="response_time" placeholder="e.g., 24 hours" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Service Area</label>
                        <input type="text" wire:model="service_area" placeholder="e.g., Mediterranean" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                    </div>
                ...
                <div>
                    ...
                    @elseif(!empty($existing_logo))
                        <img src="{{ $existing_logo }}" alt="Current" class="w-32 h-32 object-cover rounded-lg mb-2" onerror="this.style.display='none'">
                    @endif
                ...
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4 border-t border-gray-200">
                    ...
                    <a href="{{ route('industryreview.contractors.manage') }}" class="px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors text-center text-sm sm:text-base">
                        Cancel
                    </a>
                </div>
            </form>
