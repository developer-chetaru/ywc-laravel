...
<div class="p-6 bg-white rounded-lg shadow-md w-full mx-auto mt-1">
    <h2 class="text-xl font-bold text-[#0053FF] mb-6">Add Certificate Type with Issuers</h2>

    @if(session()->has('message'))
        <div class="text-[#0053FF] mb-4">{{ session('message') }}</div>
    @endif

    <form action="{{ route('certificate-type.store') }}" method="POST">
        @csrf

        <!-- Certificate Type Name -->
        <div class="mb-4">
            <label class="block text-gray-700 mb-2">Certificate Type Name</label>
            <input type="text" name="name" value="{{ old('name') }}" placeholder="Enter certificate type"
        ...
        <div class="mb-6">
            ...
            <table class="w-full border-collapse border border-gray-300" id="issuersTable">
                <thead>
                ...
                <tbody>
                    <tr>
                        <td class="border p-2">
                            <select name="issuers[0][name]" class="border p-2 rounded w-full searchable-select">
        ...
        <div class="flex gap-2">
            <button type="submit" class="px-4 py-2 bg-[#0053FF] text-white rounded hover:bg-blue-700">Save</button>
